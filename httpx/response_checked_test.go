...
func TestCompatibilityWritersDoNotFallbackAfterHeaderFailure(t *testing.T) {
	headerErr := errors.New("header writer failed")

	for name, write := range map[string]func(http.ResponseWriter){
		"json": func(w http.ResponseWriter) {
			WriteJSON(w, http.StatusCreated, map[string]string{"status": "created"})
		},
		"problem": func(w http.ResponseWriter) {
			WriteProblem(w, http.StatusBadRequest, Problem{Detail: "validation failed"})
		},
	} {
		t.Run(name, func(t *testing.T) {
			...
			if w.writes != 0 {
				t.Fatalf("writes = %d, want no fallback response", w.writes)
			}
		})
	}
}
