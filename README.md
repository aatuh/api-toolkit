...
- Release readiness requires an explicit baseline, for example `API_BASE_REF=v3.1.1 GOTOOLCHAIN=local make release-check` for a post-`v3.1.1` v3 release.
- Publication evidence requires an explicit baseline, for example `API_BASE_REF=v3.1.1 GOTOOLCHAIN=local make release-evidence`, from a clean worktree.
- `ALLOW_DIRTY_RELEASE_EVIDENCE=1 API_BASE_REF=v3.1.1 GOTOOLCHAIN=local make release-evidence` is only for local dirty-tree audit evidence and is not acceptable before publishing.
