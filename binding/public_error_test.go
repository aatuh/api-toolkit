...
func TestValidationProblemAllowsExplicitlyPublicProviderFields(t *testing.T) {
	problem := ValidationProblem(safeFieldError{message: "postal code is invalid"})
	fields, ok := problem.Ext["field_errors"].(map[string]string)
	if !ok || fields["postal_code"] != "postal code is invalid" {
		t.Fatalf("validation fields = %#v", problem.Ext)
	}
}
...
type safeFieldError struct {
	message string
}

func (e safeFieldError) Error() string { return "internal: " + e.message }

func (e safeFieldError) FieldErrors() fielderrors.FieldErrors {
	return fielderrors.FieldErrors{{
		Field:   "postal_code",
		Code:    "invalid",
		Message: e.message,
