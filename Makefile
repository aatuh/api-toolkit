...
cli-determinism-check: ## Verify atomic deterministic CLI service generation
	@(cd cmd/api-toolkit && GOWORK=off GOTOOLCHAIN="$${GOTOOLCHAIN:-local}" $(GO) test ./... -run 'TestGenerateService(IsDeterministicAcrossFreshDestinations|FailureLeavesDestinationUntouched)$$')

cli-security-check: ## Verify CLI service-generation filesystem boundaries
	@(cd cmd/api-toolkit && GOWORK=off GOTOOLCHAIN="$${GOTOOLCHAIN:-local}" $(GO) test ./... -run 'TestNewService(RejectsTraversalOutput|FilesystemBoundaryFlags|RejectsSymlinkAndSpecialDestinationsWithoutWrites|CheckAndOverwriteProtectUserFiles|RejectsHardLinkedGeneratedProject)$$')
...
reference-service-load-check: ## Compare reference-service load evidence with reviewed local budgets
	@GOTOOLCHAIN="$${GOTOOLCHAIN:-local}" GOWORK="$${GOWORK:-off}" $(GO) run ./internal/tools/referenceserviceloadcheck
...
required-checks-verify: ## Verify required-check manifest names and workflow job IDs
...
security-review-check: ## Verify threat-model and package-scoped security review documentation
	@$(GO) test ./docscheck -count=1 -run 'Test(SecurityThreatModelCoversRequiredSurfaces|PackageSecurityReviewChecklistIsPackageScopedAndComplete)$$'
...
test-redis: ## Run real Redis contract-harness tests against the local test-only endpoint
	@(cd contrib && API_TOOLKIT_TEST_REDIS_URL="$(REDIS_TEST_URL)" GOTOOLCHAIN="$${GOTOOLCHAIN:-local}" GOWORK="$(WORKSPACE)" $(GO) test -tags=redis $(REDIS_TEST_PACKAGES) -run 'Redis' -count=1)
...
benchmark-check: ## Compare benchmark allocation samples against reviewed budgets
	@GOTOOLCHAIN="$${GOTOOLCHAIN:-local}" GOWORK="$${GOWORK:-off}" GO="$(GO)" scripts/benchmark_check.sh
...
release-authenticity-check: ## Verify release identity, signatures, provenance, and tamper rejection
	@$(MAKE) release-artifact-verify-contract workflow-security-check-contract actions-audit-contract
