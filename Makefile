...
# In api-check you can set API_BASE_REF to compare to a specific tag (for example API_BASE_REF=v3.0.0).
...
release-api-check: tools ## Run fail-closed API compatibility check; requires API_BASE_REF
	@test -n "$(API_BASE_REF)" || { echo "API_BASE_REF is required for release-api-check; for v3 patch/minor releases use the latest published v3 tag, for example API_BASE_REF=v3.0.0"; exit 2; }
...
contrib-api-drift-report: tools ## Check selected contrib API drift without making contrib stable
	@test -n "$(API_BASE_REF)" || { echo "API_BASE_REF is required for contrib-api-drift-report; for v3 patch/minor releases use the latest published v3 tag, for example API_BASE_REF=v3.0.0"; exit 2; }
...
reviewer-gate: ## Run non-mutating reviewer checks plus release evidence policy preflight
	$(MAKE) audit-check
	@test -n "$(API_BASE_REF)" || { echo "API_BASE_REF is required for reviewer-gate; for v3 patch/minor releases use the latest published v3 tag, for example API_BASE_REF=v3.0.0"; exit 2; }
...
release-evidence: ## Run release readiness and write release-check-summary.json
	@test -n "$(API_BASE_REF)" || { echo "API_BASE_REF is required for release-evidence; for v3 patch/minor releases use the latest published v3 tag, for example API_BASE_REF=v3.0.0"; exit 2; }
	@tmp="$$(mktemp)"; \
	status=0; \
