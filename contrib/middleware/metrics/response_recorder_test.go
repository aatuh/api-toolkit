...
var errMetricsHijacked = errors.New("metrics hijacked")
...
func (w *optionalResponseWriter) Hijack() (net.Conn, *bufio.ReadWriter, error) {
	w.hijacked = true
	return nil, nil, errMetricsHijacked
}
...
func TestResponseRecorderTracksInformationalAndFinalStatuses(t *testing.T) {
	...
	if got := rec.Status(); got != http.StatusAccepted {
		t.Fatalf("final status = %d, want %d", got, http.StatusAccepted)
	}
	if !rec.Committed() {
...
func TestResponseRecorderWriteReadFromAndOptionalInterfaces(t *testing.T) {
	...
	if _, _, err := rec.Hijack(); !errors.Is(err, errMetricsHijacked) || !base.hijacked {
