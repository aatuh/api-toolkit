...
func HealthChecker(cfg Config, client *http.Client) ports.HealthChecker {
	...
	return health.NewCustomCheckerWithTimeout(
		...
		func(ctx context.Context) (ports.HealthStatus, string, interface{}) {
			...
			if err != nil {
				return ports.HealthStatusDegraded, fmt.Sprintf("oidc jwks request failed: %v", err), nil
			}
			...
			if resp.StatusCode != http.StatusOK {
				return ports.HealthStatusDegraded, fmt.Sprintf("unexpected status %d from oidc jwks", resp.StatusCode), map[string]interface{}{
					"status_code": resp.StatusCode,
					"url":         cfg.JWKSURL,
				}
			}
			return ports.HealthStatusHealthy, "oidc jwks reachable", map[string]interface{}{
				"status_code": resp.StatusCode,
				"url":         cfg.JWKSURL,
			}
		},
