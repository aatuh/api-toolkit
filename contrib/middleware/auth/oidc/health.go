...
func HealthChecker(cfg Config, client *http.Client) health.Checker {
	...
	return health.NewCustomCheckerWithTimeout(
		...
		func(ctx context.Context) (health.Status, string, interface{}) {
			...
			if err != nil {
				return health.StatusDegraded, fmt.Sprintf("oidc jwks request failed: %v", err), nil
			}
			...
			if resp.StatusCode != http.StatusOK {
				return health.StatusDegraded, fmt.Sprintf("unexpected status %d from oidc jwks", resp.StatusCode), map[string]interface{}{
					"status_code": resp.StatusCode,
					"url":         cfg.JWKSURL,
				}
			}
			return health.StatusHealthy, "oidc jwks reachable", map[string]interface{}{
				"status_code": resp.StatusCode,
				"url":         cfg.JWKSURL,
			}
		},
