...
| `webhook` | HMAC-verified webhook receiver with size cap. | `go run ./examples/webhook` | Demo secret is `demo-secret`. | `POST /webhooks/payment` | `202` JSON acknowledgement for a valid signature. | Store real webhook secrets securely and reject unsigned payloads. |
