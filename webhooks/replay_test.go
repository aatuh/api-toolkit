...
func TestCheckReplayWindow(t *testing.T) {
	...
	request := httptest.NewRequestWithContext(context.Background(), http.MethodPost, "/webhooks", nil)
...
func TestReceiverRejectsMissingReplayEventID(t *testing.T) {
	...
	receiver.ServeHTTP(recorder, httptest.NewRequestWithContext(context.Background(), http.MethodPost, "/webhooks", strings.NewReader(`{"ok":"yes"}`)))
