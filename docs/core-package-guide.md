...
| `middleware/timeout` | Request deadlines. | You need cooperative deadlines globally or an explicitly wrapped finite JSON hard timeout. | The route is streaming, SSE, websocket, a large download, or needs optional writer interfaces. | stable | root-only | [example](../middleware/timeout/example_test.go) |
