...
| Legacy response helpers | Public `response_writer` package removed. | `github.com/aatuh/api-toolkit/v4/httpx` and package-local response recorders. | `httpx` response tests, idempotency capture tests, and docscheck rules that examples avoid the removed package. |
