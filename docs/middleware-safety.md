...
| `middleware/timeout` hard timeout | No. Apply cooperative deadlines globally instead. | Use `HardTimeout.WrapRoute` only for finite JSON routes that need a wall-clock response cutoff. | Yes for streaming, server-sent events, websocket, large downloads, and handlers requiring `http.Flusher`, `http.Hijacker`, `http.Pusher`, or `io.ReaderFrom`. | `RouteCapabilities` rejects unsafe route declarations before wrapping. Existing `securityprofile.StreamingRouteOverride` remains a v4 compatibility opt-out. |
...
10. Cooperative timeout broadly; hard timeout only through `WrapRoute` after
    route capabilities reject streaming and optional writer requirements.
