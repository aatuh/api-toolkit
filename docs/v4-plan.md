...
| --- | --- | --- |
| `middleware/auth/jwt`, JWK handling, and OAuth2 helpers | Split to an auth module or keep behind an explicitly imported package boundary that does not affect simple middleware adopters. | Users of `httpx`, `binding`, or `middleware/maxbody` should not inherit JWT/JWK dependency weight. |
