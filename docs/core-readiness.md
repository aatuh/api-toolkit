...
| `github.com/aatuh/api-toolkit/v4/middleware/idempotency` | Package docs | Compile checked | Direct tests | `FuzzDefaultHashAndReplayMetadata` smoke | Baseline exists | Stable API gate | Raw idempotency keys and replay bodies | App owns durable store and opt-outs. |
...
| `github.com/aatuh/api-toolkit/v4/middleware/ratelimit` | Package docs | Compile checked | Direct tests | Not required | Baseline exists | Stable API gate | Bounded keys, trusted proxies, and dev bypasses | In-memory quota is per-process; app owns a distributed `DecisionLimiter` when replicas need shared enforcement. |
...
| `github.com/aatuh/api-toolkit/v4/middleware/timeout` | Package docs | Compile checked | Direct tests | Not required | Baseline exists | Stable API gate | Streaming, optional writer interfaces, and cancellation-ignoring handlers | App owns deadlines; use cooperative timeout globally and explicitly wrap only finite JSON hard-timeout routes. |
