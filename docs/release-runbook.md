...
5. `API_BASE_REF=v3.0.0 GOTOOLCHAIN=local make release-evidence` only from a clean worktree to produce local publication evidence.
...
8. `ALLOW_DIRTY_RELEASE_EVIDENCE=1 API_BASE_REF=v3.0.0 GOTOOLCHAIN=local make release-evidence` only for local audit context; never publish from this evidence.
