...
8. Review `docs/supported-adapter-test-realism.tsv` when supported adapter behavior changes. Run `make supported-adapter-check` with any supported PostgreSQL adapter change; its `real-postgres-pr` rows require the isolated `make test-postgres` contract in pull-request and release workflows.
