...
| --- | --- | --- | --- |
| Go version mismatch | Root and contrib require Go 1.25.x as the minimum and test Go 1.26.x as the current line, but the local toolchain is older or `GOTOOLCHAIN` downloads a different version. | Install Go 1.25.x or Go 1.26.x, then run local repository gates with `GOTOOLCHAIN=local`. | `go version` and `GOTOOLCHAIN=local make docs-check`. |
