...
| Redis | `adapters/cacheredis`, `adapters/idempotencyredis`, `adapters/ratelimitredis` | supported-adapter | Use for shared production cache, idempotency, and rate-limit state with service-specific prefixes and bounded keys. Real contracts run through `make test-redis` on every pull request and release. |
