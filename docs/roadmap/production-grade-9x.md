...
* Default branch `master` at
  `24c46a2efc50f8d03691047eb425122a740bd26a`.

* Release `v4.0.0` at
  `3cfc8d44423029ec50516d6b857d938b75067737`.

* The v4 release documentation still contained current-state references to
  `v3.1.2`.
...
```sh
API_BASE_REF=<latest-supported-tag> \
  GOWORK=off \
  GOTOOLCHAIN=local \
  make release-check
```
...
| ------------------- | ---------------------------------------------------------------------------------- |
| M0: Trust repair    | Default branch, release, documentation, and evidence agree.                        |
...
## [ ] REL-001: Reconcile `master` with `v4.0.0`
...
* Inventory every commit present in `v4.0.0` but absent from `master`.
* Determine whether each commit should be:

  * Merged into `master`.
  * Reapplied with a corrected commit.
  * Explicitly superseded.
* Do not move or rewrite the published `v4.0.0` tag.
* Make the release commit an ancestor of the default branch.
* Resolve any conflicts without dropping v4 API or release evidence changes.
* Document the cause of the divergence in
  `docs/release-incident-v4-branch-divergence.md`.
* Add a prevention action to the release runbook.

### Done when

```sh
git merge-base --is-ancestor v4.0.0 origin/master
```
...
* `v4.0.0` still resolves to its original commit.
...
```text
fix(release): reconcile master with v4.0.0
```
...
**Owner:** Release engineer
**Size:** M
**Depends on:** REL-001, REL-002, REL-003, REL-004, DOC-001, DOC-005

### Work

Prepare `v4.0.1` as a trust-repair patch release.
...
Publish `docs/releases/v4.0.1-verification.md` containing:
...
* The default branch README names `v4.0.1`.
* No current-state document claims v3 is the latest release.
* Root and contrib module installations resolve without `replace`.

### Verification

```sh
go list -m github.com/aatuh/api-toolkit/v4@v4.0.1
go list -m github.com/aatuh/api-toolkit/contrib/v4@v4.0.1
RELEASE_TAG=v4.0.1 \
...
TST-004, DOC-001 through DOC-005
