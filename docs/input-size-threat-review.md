...
| `middleware/timeout.Options.MaxCaptureBytes` | `middleware/timeout.NewHard` and `HardTimeout.WrapRoute` | Zero defaults to 1 MiB. Negative values fail construction. Capture overflow returns a Problem Details error instead of truncating a response. | Hard-timeout response buffering starts one handler goroutine per request, can consume the capture limit, and drops optional writer interfaces. | Apply cooperative `NewPropagator` globally. Use `WrapRoute` only for finite responses after `RouteCapabilities` rejects streaming, SSE, websocket, hijack, large-download, and optional-writer routes. |
